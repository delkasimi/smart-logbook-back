...
const getMediaBySolutionId = async (req, res) => {
  ...
  try {
    const mediaItems = await Media.findAll({
      where: { associated_id: solutionId, associated_type: "solution" },
    });
    res.json(mediaItems);
  } catch (err) {
    console.error(err);
...
const getMediaByTicketId = async (req, res) => {
  ...
  try {
    const mediaItems = await Media.findAll({
      where: { associated_id: ticketId, associated_type: "ticket" },
    });
    res.json(mediaItems);
  } catch (err) {
    console.error(err);
...
const getMediaById = async (req, res) => {
  ...
  try {
    const mediaItems = await Media.findAll({
      where: { associated_id: ticketId, associated_type: type },
    });
    res.json(mediaItems);
  } catch (err) {
    console.error(err);
